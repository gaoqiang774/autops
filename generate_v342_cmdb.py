...
# 1. Locate and Load Excel v342
excel_candidates = glob.glob(r'e:\autops\*342*.xlsx')
if not excel_candidates:
    raise FileNotFoundError("未在 e:\\autops 目录下找到 《信息资产台账-v342.xlsx》 文件！")

excel_path = excel_candidates[0]
print(f"Loading Excel from: {excel_path}")

wb = openpyxl.load_workbook(excel_path, data_only=True)
sheet_candidates = [s for s in wb.sheetnames if '02' in s or '硬件' in s]
if not sheet_candidates:
    raise ValueError(f"未找到硬件sheet页！Sheet列表: {wb.sheetnames}")
...
for i in range(min(10, len(rows))):
    ...
    if "设备名称" in text or "私有IP" in text or ("序号" in text and "项目名称" in text):
        header_idx = i
        break

if header_idx == -1:
    raise ValueError("未能识别到符合《02-硬件设备》规范的表头！")

headers = [str(c).strip() if c is not None else f"col_{i}" for i, c in enumerate(rows[header_idx])]
print(f"Header located at row index {header_idx}: {headers}")
...
for idx, r in enumerate(rows[header_idx + 1:]):
    ...
    if item.get("设备名称") or item.get("私有IP（业务IP）"):
        item["_row"] = header_idx + 1 + idx + 1
        raw_devices.append(item)

print(f"Loaded {len(raw_devices)} raw devices from {sheet_candidates[0]}")
...
// Auto-generated strictly aligned with 《信息资产台账-v342.xlsx》 sheet「02-硬件设备」
...
target_cmdb_path = r'e:\autops\app\cmdbData.ts'
with open(target_cmdb_path, 'w', encoding='utf-8') as f:
    f.write(ts_content)

print(f"Successfully generated and wrote {len(ts_content)} characters to {target_cmdb_path}")
